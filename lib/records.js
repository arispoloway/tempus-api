...
async function mapTime(map_name, c, number){
    var r = await mapRecords(map_name, "map", 1, number);
    if (c === 'd') {
        return r.demoman[number - 1];
    } else if (c === 's') {
        return r.soldier[number - 1];
    }
}

async function courseTime(map_name, course, c, number) {
    var r = await mapRecords(map_name, "course", course, number);
    if (c === 'd') {
        return r.demoman[number - 1];
    } else if (c === 's') {
        return r.soldier[number - 1];
...
async function bonusTime(map_name, bonus, c, number) {
    var r = await mapRecords(map_name, "bonus", bonus, number);
    if (c === 'd') {
        return r.demoman[number - 1];
    } else if (c === 's') {
        return r.soldier[number - 1];
